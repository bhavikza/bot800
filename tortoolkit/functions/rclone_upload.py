...
async def rclone_driver(path,message, user_msg, dl_task):
    ...
    if conf_path is None:
        torlog.info("The config file (rclone.conf) was not found")
        return None
    else:
...
async def rclone_upload(path,message,user_msg,dest_drive,dest_base,edit_time,conf_path, task):
    ...
    if os.path.isdir(path):
        ...
        buttons = []
        buttons.append(
            [KeyboardButtonUrl("View on Google Drive 🔗",folder_link)]
        )
        ...
        if gd_index:
            ...
            buttons.append(
                [KeyboardButtonUrl("View on Index ☁️",index_link)]
            )

        ul_size = calculate_size(path)
        transfer[0] += ul_size
        #ul_size = Human_Format.human_readable_bytes(ul_size)
        txtmsg = "<a href='tg://user?id={}'>Done</a>\n#uploads\nUploaded Size:- {}\nUploaded folder: <code>{}</code> To Drive.".format(omsg.sender_id,ul_size,os.path.basename(path))
        ...
        buttons.append(
            [KeyboardButtonUrl("View on Google Drive 🔗",file_link)]
        )
        gd_index = get_val("GD_INDEX_URL")
        if gd_index:
            index_link = "{}/{}".format(gd_index.strip("/"), gid[1])
        ...
        transfer[0] += ul_size
        ul_size = Human_Format.human_readable_bytes(ul_size)
        txtmsg = "<a href='tg://user?id={}'>Done</a>\n#uploads\nUploaded Size:- {}\nUploaded file:-<code>{}</code> To Drive.".format(omsg.sender_id,ul_size,os.path.basename(path))
